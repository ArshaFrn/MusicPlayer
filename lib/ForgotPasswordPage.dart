...
class _ForgotPasswordPageState extends State<ForgotPasswordPage> {
  ...
  void _showSnackBar(String message, {bool isError = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: isError ? Colors.red.withOpacity(0.65) : Color(0xFF8456FF),
  ...
  Future<void> _requestResetCode() async {
    ...
    try {
      ...
      if (response['status'] == 'resetCodeSent' || response['status'] == 'success') {
        ...
        _showSnackBar(response['message'] ?? "Failed to send reset code", isError: true);
      }
    } catch (e) {
  ...
  Future<void> _verifyResetCode() async {
    ...
    try {
      ...
      if (response['status'] == 'resetCodeVerified' || response['status'] == 'success') {
        ...
        _showSnackBar(response['message'] ?? "Invalid or expired reset code", isError: true);
      }
    } catch (e) {
      _showSnackBar("Error: $e", isError: true);
    } finally {
  ...
  Future<void> _updatePassword() async {
    ...
    if (!isPasswordValid(_newPasswordController.text)) {
      _showSnackBar("Password must be 8+ chars with uppercase, lowercase, and a number.", isError: true);
      return;
    }
    ...
    try {
      ...
      if (response['status'] == 'passwordUpdateSuccess' || response['status'] == 'success') {
        ...
        _showSnackBar(response['message'] ?? "Failed to update password", isError: true);
      }
    } catch (e) {
      _showSnackBar("Error: $e", isError: true);
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    Widget buildTextField(TextEditingController controller, String hintText, IconData icon, {bool isPassword = false}) {
      return TextField(
        ...
        decoration: InputDecoration(
          ...
          enabledBorder: OutlineInputBorder(
            ...
            borderSide: BorderSide(color: Colors.white24, width: 1.3),
          ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ShaderMask(
                        shaderCallback: (Rect bounds) {
                          return const LinearGradient(
                            colors: [Color(0xFF8456FF), Color(0xFFB388FF)],
                            begin: Alignment.topLeft,
                            end: Alignment.bottomRight,
                          ).createShader(bounds);
                        },
                        child: Text(
                          _currentStep == 'email' ? 'FORGOT PASSWORD' : _currentStep == 'code' ? 'ENTER CODE' : 'NEW PASSWORD',
                          style: TextStyle(fontSize: 32, fontWeight: FontWeight.w500, color: Colors.white, letterSpacing: 3, shadows: [Shadow(blurRadius: 15, color: Color(0xFF8456FF))]),
                          textAlign: TextAlign.center,
                        ),
                      ),
                      SizedBox(height: 20),
                      Text(
                        _currentStep == 'email' ? 'Enter your email to receive a reset code.' : _currentStep == 'code' ? 'Enter the code sent to $_resetEmail.' : 'Enter your new password.',
                      ...
                      if (_currentStep == 'email')
                        ...
                        buildTextField(_resetCodeController, 'Reset Code', Icons.security)
                      else ...[
                          buildTextField(_newPasswordController, 'New Password', Icons.lock, isPassword: true),
                          SizedBox(height: 20),
                          buildTextField(_confirmPasswordController, 'Confirm Password', Icons.lock, isPassword: true),
                        ],
                      SizedBox(height: 30),
                        ...
                        children: [
                          Expanded(
                            child: ElevatedButton(
                              onPressed: () => Navigator.of(context).pop(), // Always go back
                              style: ElevatedButton.styleFrom(backgroundColor: Colors.grey.withOpacity(0.3), foregroundColor: Colors.white, padding: EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                              child: Text('Back'),
                            ),
                          ),
                          SizedBox(width: 15),
                          Expanded(
                            child: _currentStep == 'email'
                                ? buildActionButton('Send Code', _requestResetCode)
                                : _currentStep == 'code'
                                ? buildActionButton('Verify', _verifyResetCode)
                                : buildActionButton('Update', _updatePassword),
                          ),
                        ],
                      ),
                    ],
